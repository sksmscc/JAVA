...
public class BaseBallGame {
	...
	public static void main(String[] args) {
		/* 야구게임 메서드로 변경 ...
		 * 
		 * 메서드 : 메개변수의 개수는 정해진 것이 없음.
		 * 리턴타입 => 하나
		 * 
		 * 랜덤번호 생성
		 * 배열에 추가 (중복메서드 포함할지 안할지 선택.)
		 * (스트라이크, 볼 메서드를 같이 나오게.)
		 * 스트라이크 메서드
		 * 0 1 2
		 * 3 5 4
...
//	}
	...
	public static int random(int max, int min) {
		if(max <min) {  //max와 min의 숫자 값이 바뀌어서 들어올 떄 처리
		  int tmp = max;
		  max = min;
		  min = tmp;
        }
	...
	//void로 적용하면 return 부분을 sysout 으로 표현 가능.
	public static boolean randomArray(int arr[], int max, int min) {
		if(arr == null) {
			return false;
		}
		if((max-min+1) < arr.length) {    //랜덤수의 개수가 배열의 length보다 작다면 //채울 수 있는 범위가 작음.
			return false;
		}
		int i = 0;
		while(i<arr.length) {
	...
	//스트라이크 메서드
	// 기능 :  com[], user[] 위치와 값이 일치하면 카운트 반환하기능
	//메서드명 : strike
	...
	//볼 메서드
    //기능 : com[], user[] 위치가 상관없이 일치하면 카운트 증가 반환 기능
	//메서드명 : ball
