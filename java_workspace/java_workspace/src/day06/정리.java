...
public class 정리 {
	...
	public static void main(String[] args) {
		 ...
		 * - 정수형 : int, byte, short, long(접미사 L(1))
		 * - 실수형 : double, float(접미사 f(F))
		 * - 문자형 : char(정수형으로 분류하기도 함. 유니코드표에 코드로 저장) : 1글자
		 * - 불리언 : boolean (true/false)
		 * - 참조변수 : 객체의 주소를 저장하는 변수ㅈ(객체명) : new 키위드를 통해 객체 생성
		 * - 자료형 변환 : 변수, 상수, 리터럴 값들을 일시적으로 다른 자료형으로 변환
		 *   - 자동형변환 : 정수 -> 실수, 작은 바이트 ->큰 바이트
		 *   - 명시적변환 : 자동으로 형변환이 되지 않는 경우 (자료형) 변수명
		 *   
		 * 2. 연산자
		 * - 대입연산자(=)
		 *  - 오른쪽에 있는 값을 왼쪽 변수에 저장
		 *  - 왼쪽에는 반드시 저장가능한 변수/객체가 와야 함.
		 ...
		 *     }
		 *  - do { //무조건 1번은 실행되야 하는 반복문에서 사용
		 *     실행문;
		 *     } while(조건식;)
		 *     
		 * 5. break, continue문
		 *  - break : 반복문을 빠져나기 위한 구문 / if문을 같이 동반하게 됨.
